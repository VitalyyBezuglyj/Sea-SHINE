...
class FeatureOctree(nn.Module):
    def __init__(self, config: SHINEConfig):
        ...
        if config.use_pe:
            self.pos_encoder = PositionalEncoder(config)
            self.pe = True
            self.mix_with_pe = config.mix_with_interpolation
            self.pe_levels = config.pe_levels
        elif config.use_gaussian_pe:
            self.pe = True
            self.pe_levels = config.pe_levels
            self.mix_with_pe = config.mix_with_interpolation
            self.pos_encoder = GaussianFourierFeatures(config)
        else:
            self.pe = False
            self.mix_with_pe = False
    ...
    def get_octree_nodes(self, level):  # top-down
        ...
        nodes_coord_scaled = (nodes_spc_np * node_size) - 1.0 + 0.5 * node_size  # in the -1 to 1 kaolin space
            ...
            if len(self.corners_lookup_tables[i]) == 0:  # for the first frame
                ...
                if incremental_on:
                    ...
                    self.importance_weight[cur_featured_level] = torch.cat(
                        (self.importance_weight[cur_featured_level][:-1], new_weights), 0
                    )
    ...
    def get_indices(self, coord):
        ...
        for i in range(self.featured_level_num):  # bottom-up, for each level
            ...
            indices_list = [
                self.nodes_lookup_tables[current_level].get(p, features_last_row) for p in points_morton
            ]
    ...
    def query_feature(self, coord, faster=False):
        ...
        # coord: [bs, 3]
        # features: [bs, self.feature_dim = 8]

        if self.pe:
            full_pe = torch.empty((features.shape[0], 0), device=self.device)
            for i in self.pe_levels:
                current_level = self.max_level - i
                pe = self.apply_pe(coord, current_level)
                full_pe = torch.cat((full_pe, pe), dim=1)
            features = torch.cat((features, full_pe), dim=1)
        return features
...
class GaussianFourierFeatures(nn.Module):
    def __init__(self, config: SHINEConfig) -> None:
        ...
        self.freq = torch.tensor(config.pos_encoding_freq, device=config.device)
        self.num_bands = config.pos_encoding_band
        self.dimensionality = config.pos_input_dim

        self.register_buffer(
            "B", torch.randn([self.dimensionality, self.num_bands], device=config.device) * self.freq
        )
