...
from model.decoder import Decoder, get_geo_decoder
...
def run_shine_mapping_batch():
    ...
    shutil.copy2(sys.argv[1], run_path.as_posix())  # copy the config file to the result folder
    ...
    # geo_mlp = Decoder(config, is_geo_encoder=True, is_time_conditioned=config.time_conditioned, semantic_aware=config.semantic_aware)
    geo_mlp = get_geo_decoder(config)
    ...
    if config.load_model:
        ...
        if "feature_octree" in loaded_model.keys() and config.load_tree:  # also load the feature octree
    ...
    if config.use_outliers:
        outlier_dataset = LiDARDataset(config, octree, use_outliers=True)

    mesher = Mesher(config, octree, geo_mlp, sem_mlp)
    dataset.begin_pose_inv = np.eye(4)
    ...
    for frame_id in tqdm(range(dataset.total_pc_count)):
        ...
        if config.use_outliers:
            outlier_dataset.process_frame(frame_id)
        t1 = get_time()
    ...
    dataset.write_merged_pc(pc_map_path.as_posix())
    ...
    if (
        config.normal_loss_on
        or config.ekional_loss_on
    ...
    for iter in tqdm(range(config.iters)):
        ...
        if config.ray_loss:  # loss computed based on each ray
            ...
            if config.use_outliers and config.outlier_start_iter < iter:
                coord_o, sample_depth_o, ray_depth_o, normal_label_o, sem_label_o, weight_o = (
                    outlier_dataset.get_batch()
                )
        else:  # loss computed based on each point sample
            coord, sdf_label, origin, ts, normal_label, sem_label, weight = dataset.get_batch()
            if config.use_outliers and config.outlier_start_iter < iter:
                coord_o, sdf_label_o, origin_o, ts_o, normal_label_o, sem_label_o, weight_o = (
                    outlier_dataset.get_batch()
                )
        ...
        if config.semantic_on:
            ...
            if config.use_outliers and config.outlier_start_iter < iter:
                sem_label_o = sem_mlp.sem_label(feature_o)

        if config.time_conditioned:
            pred = geo_mlp.time_conditionded_sdf(feature, ts)  # predict the scaled sdf with the feature

        elif config.semantic_aware:
            pred = geo_mlp.sea_sdf(feature, sem_label.to(feature.device))

            if config.use_outliers and config.outlier_start_iter < iter:
                pred_o = geo_mlp.sea_sdf(feature_o, sem_label_o.to(feature.device))

            if config.semantic_decomposition_loss:
                fake_labels = generate_new_labels(sem_label, num_classes=config.sem_class_count + 1)
                wrong_pred = geo_mlp.sea_sdf(feature, fake_labels.to(feature.device))
        else:
        ...
        if require_gradient:
            ...
            if config.use_outliers and config.outlier_start_iter < iter:
                g_o = get_gradient(coord_o, pred_o) * sigma_sigmoid
        ...
        if config.consistency_loss_on:
            near_index = torch.randint(
                0, coord.shape[0], (min(config.consistency_count, coord.shape[0]),), device=dev
            )
        ...
        if config.ray_loss:  # neural rendering loss
            ...
            if config.main_loss_type == "sdf_bce":  # our proposed method
                sdf_loss = sdf_bce_loss(
                    pred, sdf_label, sigma_sigmoid, weight, config.loss_weight_on, config.loss_reduction
                )
                if config.use_outliers and config.outlier_start_iter < iter:
                    sdf_loss_o = sdf_bce_loss(
                        pred_o,
                        sdf_label_o,
                        sigma_sigmoid,
                        weight_o,
        ...
        if config.ekional_loss_on:
            ...
            if config.use_outliers and config.outlier_start_iter < iter:
                eikonal_loss_o = (
                    (1.0 - g_o[surface_mask].norm(2, dim=-1)) ** 2
                ).mean()  # MSE with regards to 1
                cur_loss += config.weight_e * eikonal_loss_o
        ...
        if config.semantic_on:
            ...
            sem_loss = loss_nll(
                sem_pred[:: config.sem_label_decimation, :], sem_label[:: config.sem_label_decimation]
            )
            cur_loss += config.weight_s * sem_loss

        semantic_decomposition_loss = 0.0
        if config.semantic_aware and config.semantic_decomposition_loss:
            weight = torch.abs(
                weight
            )  # weight's sign indicate the sample is around the surface or in the free space
            if config.main_loss_type == "sdf_bce":  # our proposed method
                semantic_decomposition_loss = contrastive_sdf_bce_loss(
                    wrong_pred,
                    sdf_label,
                    sigma_sigmoid,
                    weight,
                ...
                semantic_decomposition_loss = sdf_diff_loss(
                    wrong_pred, sdf_label, weight, config.scale, l2_loss=False
                )
            elif config.main_loss_type == "sdf_l2":
                semantic_decomposition_loss = sdf_diff_loss(
                    wrong_pred, sdf_label, weight, config.scale, l2_loss=True
                )
            cur_loss += config.contrastive_w * semantic_decomposition_loss
        ...
        if config.wandb_vis_on:
            if config.ray_loss:
                ...
                wandb_log_content = {
                    ...
                    "loss/contrastive": semantic_decomposition_loss,
                }
        ...
        if ((iter + 1) % config.vis_freq_iters) == 0 and iter > 0:
            ...
            if not config.time_conditioned:
                mesh_path = run_path / f"mesh/mesh_iter_{str(iter + 1)}.ply"
                map_path = run_path / f"map/sdf_map_iter_{str(iter + 1)}.ply"
                if config.mc_with_octree:  # default
                    cur_mesh = mesher.recon_octree_mesh(
                        config.mc_query_level,
                        config.mc_res_m,
                        mesh_path.as_posix(),
                        map_path.as_posix(),
                        config.save_map,
                    ...
                    cur_mesh = mesher.recon_bbx_mesh(
                        dataset.map_bbx,
                        config.mc_res_m,
                        mesh_path,
                        map_path,
                ...
                for frame_id in tqdm(range(dataset.total_pc_count)):
                    if frame_id < config.begin_frame or frame_id > config.end_frame or frame_id % 2 != 0:
                        continue
                    mesher.ts = frame_id
                    mesh_path = run_path / f"mesh/mesh_iter_{str(iter + 1)}_ts_{str(frame_id)}.ply"
                    map_path = run_path / f"map/sdf_map_iter_{str(iter + 1)}_ts_{str(frame_id)}.ply"
                    if config.mc_with_octree:  # default
                        cur_mesh = mesher.recon_octree_mesh(
                            config.mc_query_level,
                            config.mc_res_m,
