    ...
    for frame_id in tqdm(range(dataset.total_pc_count)):
        ...
        for iter in tqdm(range(config.iters)):
            ...
            sdf_loss = sdf_bce_loss(
                sdf_pred, sdf_label, sigma_sigmoid, weight, config.loss_weight_on, config.loss_reduction
            )
            cur_loss += sdf_loss
            ...
            if config.ekional_loss_on:  # MSE with regards to 1
                ...
                eikonal_loss = (
                    (g[surface_mask].norm(2, dim=-1) - 1.0) ** 2
                ).mean()  # only close to the surface
        ...
        if config.continual_learning_reg:
            opt.zero_grad(set_to_none=True)
            cal_feature_importance(
                dataset,
                octree,
                geo_mlp,
                sigma_sigmoid,
                config.bs,
                config.cal_importance_weight_down_rate,
        ...
        if processed_frame == 0 or (processed_frame + 1) % config.mesh_freq_frame == 0:
            ...
            elif config.mc_local:  # only build the local mesh to speed up
