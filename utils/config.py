...
class SHINEConfig:
    def __init__(self):
        ...
        self.kitti360: bool = False

        self.load_model: bool = False  # load the pre-trained model or not
        self.load_mlp: bool = True  # does not override load_model - just choose what to load if load
        self.load_tree: bool = True  # does not override load_model - just choose what to load if load
        self.model_path: str = "/"  # pre-trained model path
        self.sem_model_path: str = "/"
        ...
        self.global_shift_default: float = 0.0  # 0.17241
        ...
        self.rand_down_way: str = "random"
        self.min_down_r: float = 0.1
        self.with_contour: bool = False
        self.contour_mode: str = "semantic"  # "semantic" or "distance"
        self.contour_delta: float = 0.3  # only for "distance" mode
        self.add_distance_noise: float = False
        self.dist_noise_std: float = 0.03
        self.dist_noise_rel_noise: float = 0.004
        self.add_points_dropout: float = False
        self.use_outliers: bool = False
        self.outlier_start_iter: int = 1e6
        ...
        self.mix_with_interpolation: bool = False
        self.pe_levels: list = [0]
        self.use_pe: bool = False
        ...
        self.pos_encoding_band: int = 20  # if 0, without encoding
        self.pos_input_dim: int = 3
        self.pos_encoding_base: int = 2

        # semantic related
        self.semantic_on: bool = True  # semantic shine mapping on [semantic]
        self.sem_class_count: int = 20  # semantic class count: 20 for semantic kitti
        self.sem_label_decimation: int = (
            1  # use only 1/${sem_label_decimation} of the available semantic labels for training (fitting)
        )
        self.filter_moving_object: bool = False
        self.semantic_aware: bool = False
        ...
        self.sigma_scale_constant: float = (
            0.0  # scale factor adding to the constant sigma value (linear with the distance) [deprecated]
        )
        self.logistic_gaussian_ratio: float = 0.55

        self.proj_correction_on: bool = (
            False  # conduct projective distance correction based on the sdf gradient or not [deprecated]
        )
        ...
        self.semantic_decomposition_loss: bool = False
        self.contrastive_loss_margin: float = 1.0
        self.contrastive_w: float = 0.1
        ...
        self.consistency_range: float = (
            0.1  # the neighborhood points would be randomly select within the radius of xxx m
        )
        ...
        self.o3d_vis_on: bool = (
            True  # visualize the mesh in-the-fly using o3d visualzier or not [press space to pasue/resume]
        )
        ...
        self.min_cluster_vertices: int = (
            50  # if a connected's vertices number is smaller than this value, it would get filtered
        )
        ...
        self.save_map: bool = (
            False  # save the sdf map or not, the sdf would be saved in the intensity channel
        )
    ...
    def load(self, config_file):
        ...
        self.itlp_campus = config_args["setting"].get("itlp_campus", False)
        ...
        self.kitti360 = config_args["setting"].get("kitti360", False)
        ...
        self.load_mlp = config_args["setting"].get("load_mlp", True)
        self.load_tree = config_args["setting"].get("load_tree", True)
        self.model_path = config_args["setting"]["model_path"]
        self.sem_model_path = config_args["setting"]["sem_model_path"]
        self.geo_model_path = config_args["setting"]["geo_model_path"]
        ...
        self.rand_down_way = config_args["process"].get("rand_down_way", "random")
        self.min_down_r = config_args["process"].get("min_down_r", 0.1)
        self.with_contour = config_args["process"].get("with_contour", False)
        self.contour_mode = config_args["process"].get("contour_mode", "semantic")  # "semantic" or "distance"
        self.contour_delta = config_args["process"].get("contour_delta", 0.3)  # only for "distance" mode
        ...
        self.add_distance_noise = config_args["process"].get("add_distance_noise", False)
        self.dist_noise_std = config_args["process"].get("dist_noise_std", 0.03)
        self.dist_noise_rel_noise = config_args["process"].get("dist_noise_rel_noise", 0.004)
        self.use_outliers = config_args["process"].get("use_outliers", False)
        self.outlier_start_iter = config_args["process"].get("outlier_start_iter", 1e6)

        self.add_points_dropout = config_args["process"].get("add_points_dropout", False)
        ...
        self.semantic_aware = config_args["semantic"]["semantic_aware"]
        ...
        self.decoder_type = config_args["decoder"]["decoder_type"]

        # ? positional encoding related [not used]
        self.mix_with_interpolation = config_args["pos_enc"].get("mix_with_interpolation", False)
        self.pe_levels = config_args["pos_enc"].get("pe_level", [0, 1])
        self.use_pe = config_args["pos_enc"].get("use_pe", False)
        self.use_gaussian_pe = config_args["pos_enc"].get(
            "use_gaussian_pe", False
        )  # use Gaussian Fourier or original log positional encoding
        self.pos_encoding_freq = config_args["pos_enc"].get("pos_encoding_freq", 200)  # 200
        self.pos_encoding_band = config_args["pos_enc"].get("pos_encoding_band", 20)  # if 0, without encoding
        self.pos_input_dim = config_args["pos_enc"].get("pos_input_dim", 3)
        self.pos_encoding_base = config_args["pos_enc"].get("pos_encoding_base", 2)
        ...
        self.ekional_loss_on = config_args["loss"][
            "ekional_loss_on"
        ]  # use ekional loss (gradient norm = 1) or not
        self.weight_e = float(config_args["loss"]["weight_e"])  # weight for the ekional loss

        self.semantic_decomposition_loss = config_args["loss"]["semantic_decomposition_loss"]
        self.contrastive_loss_margin = float(config_args["loss"]["contrastive_loss_margin"])
        self.contrastive_w = float(config_args["loss"]["contrastive_w"])
