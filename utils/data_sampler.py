...
class dataSampler:
    ...
    def sample(self, points_torch, sensor_origin_torch, normal_torch, sem_label_torch):
        ...
        surface_sample_displacement = (
            (torch.rand(point_num * surface_sample_n, 1, device=dev) - 0.5) * 2 * surface_sample_range_scaled
        )
        ...
        clearance_sample_displacement = (
            -torch.rand(point_num * clearance_sample_n, 1, device=dev) * clearance_dist_scaled
            - surface_sample_range_scaled
        )

        repeated_dist = distances.repeat(clearance_sample_n, 1)
        clearance_sample_dist_ratio = (
            clearance_sample_displacement / repeated_dist + 1.0
        )  # 1.0 means on the surface
        if sem_label_torch is not None:
        ...
        free_sample_dist_ratio = (
            torch.rand(point_num * freespace_sample_n, 1, device=dev) * free_diff_ratio + free_min_ratio
        )
        ...
        all_sample_displacement = torch.cat(
            (surface_sample_displacement, clearance_sample_displacement, free_sample_displacement), 0
        )
        all_sample_dist_ratio = torch.cat(
            (surface_sample_dist_ratio, clearance_sample_dist_ratio, free_sample_dist_ratio), 0
        )
        ...
        if sem_label_torch is not None:
            sem_label_tensor = torch.cat(
                (surface_sem_label_tensor, clearance_sem_label_tensor, free_sem_label_tensor), 0
            ).int()
        ...
        if normal_torch is not None:
            normal_label_tensor = (
                normal_label_tensor.reshape(all_sample_n, -1, 3).transpose(0, 1).reshape(-1, 3)
            )
        if sem_label_torch is not None:
    ...
    def sapce_carving_sample(
        self, points_torch, sensor_origin_torch, space_carving_level, stop_depth_thre, inter_dist_thre
    ):
        shift_points = points_torch - sensor_origin_torch
        ...
        mask = (
            (depth[:, 0] > 3.0)
            & (depth[:, 1] < stop_depth[nugs_ridx.long()])
            & ((depth[:, 1] - depth[:, 0]) > inter_dist_thre)
        )

        steps = torch.rand(
            mask.sum().item(), 1
        ).cuda()  # randomly sample one point on each intersected segment
        ...
        space_carving_samples = origins + directions * (
            (depth[mask, 0] + steps.reshape(1, -1) * depth_range).reshape(-1, 1)
        )
