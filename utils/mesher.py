...
class Mesher:
    def __init__(
        ...
        geo_decoder: Decoder,
        sem_decoder: Decoder,
        sem_octree: FeatureOctree = None,
    ):
        self.config = config

        self.octree = octree
        self.sem_octree = sem_octree
        ...
        with torch.no_grad():  # eval step
            if iter_n > 1:
                for n in tqdm(range(iter_n)):
                    ...
                    if self.sem_octree is not None:
                        batch_sem_feature = self.sem_octree.query_feature(batch_coord, True)
                        batch_feature = torch.cat([batch_feature, batch_sem_feature], dim=1)
                    if query_sdf:
                        if self.config.time_conditioned:
                            batch_sdf = -self.geo_decoder.time_conditionded_sdf(
                                batch_feature, self.ts * torch.ones(batch_feature.shape[0], 1).cuda()
                            )
                        elif self.config.semantic_aware:
                            _batch_sem = self.sem_decoder.sem_label(batch_feature)
                            batch_sdf = -self.geo_decoder.sea_sdf(
                                batch_feature, _batch_sem
                            )
                        else:
                    ...
                    if query_sem:
                        if self.sem_octree is not None:
                            batch_sem = self.sem_decoder.sem_label(batch_sem_feature)
                ...
                if self.sem_octree is not None:
                    sem_feature = self.sem_octree.query_feature(coord, True)
                    feature = torch.cat([feature, sem_feature], dim=1)
                if query_sdf:
                    if self.config.time_conditioned:
                        sdf_pred = (
                            -self.geo_decoder.time_conditionded_sdf(
                                feature, self.ts * torch.ones(feature.shape[0], 1).cuda()
                            )
                            .detach()
                            .cpu()
                            .numpy()
                         ...
                         _sem_pred = self.sem_decoder.sem_label(feature)
                         sdf_pred = -self.geo_decoder.sea_sdf(feature, _sem_pred).detach().cpu().numpy()
                    else:  # just for a quick test
                        sdf_pred = -self.geo_decoder.sdf(feature).detach().cpu().numpy()
                if query_sem:
                    if self.sem_octree is not None:
                        sem_pred = self.sem_decoder.sem_label(sem_feature).detach().cpu().numpy()
                    else:
    ...
    def generate_sdf_map(self, coord, sdf_pred, mc_mask, map_path):
        ...
        sdf_pred_world = (
            sdf_pred * self.config.logistic_gaussian_ratio * self.config.sigma_sigmoid_m
        )  # convert to unit: m
        ...
        sdf_map_pc.point["intensities"] = o3d.core.Tensor(
            np.expand_dims(sdf_pred_world, axis=1), dtype, device
    ...
    def recon_bbx_mesh(
        ...
        sdf_pred, _, mc_mask = self.query_points(
            coord, self.config.infer_bs, True, False, self.config.mc_mask_on
        )
        if save_map:
        ...
        mesh = o3d.geometry.TriangleMesh(
            o3d.utility.Vector3dVector(verts.astype(np.float64)), o3d.utility.Vector3iVector(faces)
        )
    ...
    def recon_octree_mesh(
        ...
        voxel_count_per_side = ((max_nodes - min_nodes) / mc_res_scaled + voxel_count_per_side_node).astype(
            int
        )
        # initialize the whole map
        query_grid_sdf = np.zeros(
            (voxel_count_per_side[0], voxel_count_per_side[1], voxel_count_per_side[2]), dtype=np.float16
        )  # use float16 to save memory
        query_grid_mask = np.zeros(
            (voxel_count_per_side[0], voxel_count_per_side[1], voxel_count_per_side[2]), dtype=bool
        )  # mask off

        for node_idx in tqdm(range(nodes_count)):
            node_coord_scaled = nodes_coord_scaled[node_idx, :]
            cur_origin = torch.tensor(
                node_coord_scaled - 0.5 * (node_res_scaled - mc_res_scaled), device=self.device
            )
            cur_coord = coord.clone()
            cur_coord += cur_origin
            cur_sdf_pred, _, cur_mc_mask = self.query_points(
                cur_coord, self.config.infer_bs, True, False, self.config.mc_mask_on
            )
